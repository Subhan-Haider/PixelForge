...
if(WIN32 AND
  ...
  if(NOT "${skia_target_cpu_line}" EQUAL "")
    # TODO We should say "laf" instead of "PixelForge", but for clarity
    #      with PixelForge users we just say "PixelForge"
    set(skia_target_cpu "${CMAKE_MATCH_1}")
    string(TOLOWER "${MSVC_C_ARCHITECTURE_ID}" pixelforge_target_cpu)

    if(NOT "${pixelforge_target_cpu}" STREQUAL "${skia_target_cpu}")
      message(FATAL_ERROR
        "----------------------------------------------------------------------"
        "                        INCOMPATIBILITY ERROR "
        "----------------------------------------------------------------------\n"
        " The specified Skia library (${SKIA_LIBRARY_DIR}) is compiled\n"
        " for ${skia_target_cpu} CPU arch but PixelForge is being targeted for ${pixelforge_target_cpu} CPU arch.\n"
        " \n"
        " Change Skia to the ${pixelforge_target_cpu} version, or start the developer prompt for ${skia_target_cpu}:\n"
...
if(NOT ZLIB_LIBRARIES)
  ...
  ExternalProject_Add(zlib-project
    GIT_REPOSITORY   https://github.com/pixelforge/zlib.git
