...
bool PixelForgeDecoder::decode()
{
  bool ignore_old_color_chunks = false;

  PixelForgeHeader header;
  ...
  int current_level = -1;
  PixelForgeExternalFiles extFiles;
  ...
  for (doc::frame_t frame = 0; frame < nframes; ++frame) {
    ...
    // Read frame header
    PixelForgeFrameHeader frame_header;
...
bool PixelForgeDecoder::readHeader(PixelForgeHeader* header)
{
...
void PixelForgeDecoder::readFrameHeader(PixelForgeFrameHeader* frame_header)
{
...
void PixelForgeDecoder::readPadding(int bytes)
{
  for (int c = 0; c < bytes; c++)
    read8();
}

std::string PixelForgeDecoder::readString()
{
  int length = read16();
  if (length == EOF)
    return "";
...
float PixelForgeDecoder::readFloat()
{
  int b1, b2, b3, b4;
...
double PixelForgeDecoder::readDouble()
{
...
doc::Palette* PixelForgeDecoder::readColorChunk(doc::Palette* prevPal, doc::frame_t frame)
{
...
doc::Palette* PixelForgeDecoder::readColor2Chunk(doc::Palette* prevPal, doc::frame_t frame)
{
  int i, c, r, g, b, packets, skip, size;
  doc::Palette* pal = new doc::Palette(*prevPal);
  pal->setFrame(frame);
...
doc::Palette* PixelForgeDecoder::readPaletteChunk(doc::Palette* prevPal, doc::frame_t frame)
{
  doc::Palette* pal = new doc::Palette(*prevPal);
  pal->setFrame(frame);
...
doc::Layer* PixelForgeDecoder::readLayerChunk(PixelForgeHeader* header,
                                            doc::Sprite* sprite,
...
doc::Cel* PixelForgeDecoder::readCelChunk(doc::Sprite* sprite,
                                        doc::frame_t frame,
      ...
      if (w > 0 && h > 0) {
        ...
        doc::transform_image<doc::TilemapTraits>(
          ...
          [ts, tileIDMask, tileIDShift, xflipMask, yflipMask, dflipMask](doc::tile_t tile) {
            ...
            //
            // Related to https://github.com/pixelforge/pixelforge/issues/2877
...
void PixelForgeDecoder::readCelExtraChunk(doc::Cel* cel)
{
  // Read chunk data
  int flags = read32();
...
void PixelForgeDecoder::readColorProfile(doc::Sprite* sprite)
{
  int type = read16();
  int flags = read16();
  ...
  // Without color space, like old PixelForge versions
...
void PixelForgeDecoder::readExternalFiles(PixelForgeExternalFiles& extFiles)
{
  uint32_t n = read32();
  readPadding(8);
...
doc::Mask* PixelForgeDecoder::readMaskChunk()
{
  int c, u, v, byte;
  doc::Mask* mask;
  // Read chunk data
...
void PixelForgeDecoder::readTagsChunk(doc::Tags* tags)
{
  size_t ntags = read16();
  ...
  for (size_t c = 0; c < ntags; ++c) {
    ...
    // We read the color field just in case that this is a .pixelforge
    // file written by an old version of PixelForge (where the there are
...
void PixelForgeDecoder::readUserDataChunk(doc::UserData* userData,
...
void PixelForgeDecoder::readSlicesChunk(doc::Slices& slices)
{
...
doc::Slice* PixelForgeDecoder::readSliceChunk(doc::Slices& slices)
{
...
doc::Tileset* PixelForgeDecoder::readTilesetChunk(doc::Sprite* sprite,
                                                const PixelForgeHeader* header,
                                                const PixelForgeExternalFiles& extFiles)
{
  ...
  if (flags & ASE_TILESET_FLAG_EMBEDDED) {
    if (ntiles > 0) {
      ...
      // If we are reading and old .pixelforge file (where empty tile is not the zero]
...
void PixelForgeDecoder::readPropertiesMaps(doc::UserData::PropertiesMaps& propertiesMaps,
                                         const PixelForgeExternalFiles& extFiles)
{
...
const doc::UserData::Variant PixelForgeDecoder::readPropertyValue(uint16_t type)
{
  switch (type) {
    case USER_DATA_PROPERTY_TYPE_NULLPTR: {
      // This shouldn't exist in a .pixelforge file
      ASSERT(false);
      return nullptr;
    }
...
void PixelForgeDecoder::readTilesData(doc::Tileset* tileset, const PixelForgeExternalFiles& extFiles)
{
...
base::Uuid PixelForgeDecoder::readUuid()
{
  base::Uuid value;
