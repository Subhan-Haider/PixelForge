...
PixelForgeUpdate::PixelForgeUpdate(std::string version)
  ...
  , m_timer(500, this)
{
  ...
  log(base::string_printf("Downloading PixelForge %s...", version.c_str()));
...
void PixelForgeUpdate::onBeforeClose(ui::CloseEvent& ev)
{
...
void PixelForgeUpdate::onDataReceived(long total, long now)
{
  ui::execute_from_ui_thread([this, total, now] { progress()->setValue(100 * now / total); });
}

void PixelForgeUpdate::onDownloadFailed(drm::LicenseManager::DownloadException& e)
{
  ui::execute_from_ui_thread([this, e] { log(e.what()); });
}

void PixelForgeUpdate::onDownloadFinished(drm::Package& package)
{
...
void PixelForgeUpdate::onInstallationPhaseStarted(drm::InstallationPhase phase)
{
...
void PixelForgeUpdate::onInstallationPhaseSkipped(drm::InstallationPhase phase)
{
  ui::execute_from_ui_thread([this, phase] {
    std::string msg = "";
    switch (phase) {
...
void PixelForgeUpdate::onInstallationPhaseFinished(drm::InstallationPhase phase)
{
  ui::execute_from_ui_thread([this, phase] {
    std::string msg;
...
void PixelForgeUpdate::onInstallationStarted(drm::Package& package)
{
  ui::execute_from_ui_thread([this] { log("Installation process started..."); });
}

void PixelForgeUpdate::onInstallationProgress(int pctCompleted)
{
  ui::execute_from_ui_thread([this, pctCompleted] { progress()->setValue(pctCompleted); });
}

void PixelForgeUpdate::onInstallationFinished(drm::Package& package)
{
...
void PixelForgeUpdate::onInstallationFailed(drm::LicenseManager::InstallationException& e)
{
  ui::execute_from_ui_thread([this, e] { log(e.what()); });
}

void PixelForgeUpdate::onTimerTick()
{
...
void PixelForgeUpdate::log(std::string text)
{
  if (m_closing)
    return;
